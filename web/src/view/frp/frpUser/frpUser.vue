  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="上游ID" prop="upId" width="120" />

        <el-table-column align="left" label="UUID" prop="uuid" width="380" />
        ...
        <el-table-column :min-width="appStore.operateMinWith" align="left" fixed="right" label="操作">
          <template #default="scope">
            ...
            <el-button class="table-button" icon="edit" link type="primary" @click="updateFrpUserFunc(scope.row)">编辑
            </el-button>
    ...
    <el-drawer v-model="dialogFormVisible" :before-close="closeDialog" :show-close="false" :size="appStore.drawerSize"
      ...
      <el-form ref="elFormRef" :model="formData" :rules="rule" label-position="top" label-width="80px">
        <el-form-item label="上游ID:" prop="upId">
          <el-input v-model="formData.upId" :clearable="true" placeholder="请输入上游ID" />
        </el-form-item>
    ...
    <el-drawer v-model="detailShow" :before-close="closeDetailShow" :show-close="true" :size="appStore.drawerSize"
      ...
      <el-descriptions :column="1" border>
        <el-descriptions-item label="上游ID">
          {{ detailFrom.upId }}
...
<script setup>
  ...
  const getTableData = async () => {
    const table = await getFrpUserList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
    if (table.code === 0) {
  ...
  const onDelete = async () => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
      ...
      const res = await deleteFrpUserByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
  ...
  // 更新行
  const updateFrpUserFunc = async (row) => {
    const res = await findFrpUser({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
  ...
  // 删除行
  const deleteFrpUserFunc = async (row) => {
    const res = await deleteFrpUser({ ID: row.ID })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功'
  ...
  const enterDialog = async () => {
    ...
    elFormRef.value?.validate(async (valid) => {
      ...
      switch (type.value) {
        case 'create':
          res = await createFrpUser(formData.value)
          break
        case 'update':
          res = await updateFrpUser(formData.value)
          break
        default:
          res = await createFrpUser(formData.value)
          break
